...
-- Enhanced Version - Player List + Fish Catch Detection
...
local LogService = game:GetService("LogService") -- For potential debugging, but mainly using chat frame monitoring
...
-- Rarity Configuration (Customizable based on levels/tiers)
local rarityConfig = {
    ["Common"] = {color = 10066329, emoji = "🟢"}, -- Grayish green
    ["Uncommon"] = {color = 32768, emoji = "🟡"}, -- Green
    ["Rare"] = {color = 255, emoji = "🔵"}, -- Blue
    ["Epic"] = {color = 8388736, emoji = "🟣"}, -- Purple
    ["Legendary"] = {color = 16737792, emoji = "🟠"}, -- Orange
    ["Mythic"] = {color = 16711680, emoji = "🔴"}, -- Red
    -- Add more rarities as needed based on game tiers
    ["Default"] = {color = 10066329, emoji = "⚪"} -- Fallback
}
...
-- Function to send player list webhook
local function sendPlayerWebhook(url, isTest)
    local players = Players:GetPlayers()
    local playerList = {}
...
-- Function to send fish catch webhook
local function sendFishWebhook(url, playerName, fishName, rarity)
    local config = rarityConfig[rarity] or rarityConfig["Default"]
    local embed = {
        ["title"] = "🐟 Fish Caught!",
        ["description"] = playerName .. " caught a " .. rarity .. " " .. fishName .. "!",
        ["color"] = config.color,
        ["fields"] = {
            {
                ["name"] = "Player",
...
-- Function to monitor chat for fish catches
local function monitorChat()
    local chatGui = LocalPlayer:WaitForChild("PlayerGui"):WaitForChild("Chat")
    local scroller = chatGui.Frame.ChatChannelParentFrame.Frame_MessageLogDisplay.Scroller
    
    scroller.ChildAdded:Connect(function(child)
        if child:IsA("Frame") and monitoringEnabled then
            local messageLabel = child:FindFirstChildWhichIsA("TextLabel", true)
            if messageLabel then
                local msg = messageLabel.Text:gsub("^%s+", ""):gsub("%s+$", "") -- Trim whitespace
                
                -- Pattern matching for fish catch messages
                -- Assuming format like: "PlayerName caught a Rarity FishName!" or "You caught a Rarity FishName!"
                local playerName, rarity, fishName = msg:match("^(.-) caught a (.-) (.-)!$")
                
                if playerName and rarity and fishName then
                    if playerName == "You" then
                        playerName = LocalPlayer.Name
                    end
                    
                    -- Check if rarity exists in config, else default
                    if not rarityConfig[rarity] then
                        rarity = "Default"
                    end
                    
                    -- Send notification in Roblox
                    showNotification("🐟 Fish Caught!", playerName .. " caught a " .. rarity .. " " .. fishName .. "!", 5)
                    
                    -- Send to Discord if webhook is set
                    if webhookURL ~= "" then
                        local success = sendFishWebhook(webhookURL, playerName, fishName, rarity)
                        if not success then
                            showNotification("⚠ Warning", "Failed to send fish catch webhook!", 3)
                        end
                    end
                end
            end
...
gui.TestButton.MouseButton1Click:Connect(function()
    ...
    local success, response = sendPlayerWebhook(url, true)
...
gui.ToggleButton.MouseButton1Click:Connect(function()
    if not monitoringEnabled then
        ...
        gui.StatusLabel.Text = "✅ Status: Active - Monitoring players & catches"
...
-- Monitor and send player webhooks
spawn(function()
    while wait(1) do
        ...
        if monitoringEnabled then
            ...
            if currentTime - lastNotificationTime >= notificationInterval then
                local success = sendPlayerWebhook(webhookURL, false)
...
wait(0.1)
showNotification("✅ Fish It Monitor", "Enhanced monitoring enabled! (Players + Fish Catches)", 5)

print("=====================================================")
print("Fish It Player Monitor - ENHANCED VERSION")
print("=====================================================")
print("✅ Player list monitoring")
print("✅ Fish catch detection from chat")
print("✅ Discord webhook notifications for players & catches")
print("✅ Auto-update every 2 seconds")
print("📊 Shows: Player names & display names")
print("🐟 Detects: Fish catches with rarity-based notifications")
